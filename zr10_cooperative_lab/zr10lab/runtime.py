"""运行编排：检测→射线→关联定位→轨迹→策略→动作→记录。

视频、推理、云台连接与 CSV 写盘各自独立。主循环不等待逐设备 ACK。
这里提供软实时调度并如实记录超期；不承诺 Windows/Python 硬实时。
"""
...
class Experiment:
    ...
    def process(self,t,dt,states,detections,history=None,truth=None,action_transform=None,
        ...
        # 单站图像跟踪可以在尚未测量多站外参时运行。它仍需要本机ray做角度
        # 补偿，但这些ray没有资格构造世界坐标定位，必须在融合入口再次隔离。
        ...
        # 真值派生指标也禁止泄漏到决策上下文。
        public_metrics = {k:v for k,v in measured_metrics.items() if k.startswith("roi_") and v is not None}
        context = PolicyContext(t,dt,self.step,states,tuple(valid_detections),tuple(rays),tuple(tracks),public_metrics)
        decision = self.policy.decide(context)
        # 控制中心在此完成手动接管/单台停机仲裁，再统一验证和记录最终动作。
        # 命令发送仍由外层运行器完成；策略返回并不代表指令已发送或设备已到位。
        ...
        self.recorder.write("cycles",t,step=self.step,dt_s=dt,work_ms=elapsed_ms,
            ...
            metrics_json=dumps(measured_metrics),diagnostics_json=dumps({"fusion":diagnostics,"policy":decision.diagnostics}))
        self.latest = {"t":t,"step":self.step,"devices":states,"tracks":tracks,"metrics":measured_metrics,
                       ...
                       "diagnostics":{"fusion":diagnostics,"policy":decision.diagnostics}}
        self.step += 1
        return actions
...
async def run_hardware(cfg,output=None,armed=False,progress=None):
    ...
    if len(cfg.active_devices) < 2:
        raise ValueError("实机协同定位需要至少两台启用设备")
    ...
    try:
        ...
        required = len(cfg.active_devices) if cfg.system.get("require_all_devices",True) else 2
        ...
        end = deadline+float(cfg.system.get("duration_s",60))
        while time.monotonic()<end:
            now = time.monotonic()
            # 数值计算和用户策略在线程中运行，保留事件循环处理遥测及看门狗的机会。
            # 姿态历史先在事件循环线程快照，避免跨线程迭代正在变化的 deque。
